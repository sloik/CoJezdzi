...
struct KrakowApiResponseParser {
    ...
    func parseApiData(data: Data?,
                      ...
                      mapping: @escaping ( (KrakowVehicle) -> TrueVehicle? ),
        ...
        guard let json = try? JSONSerialization.jsonObject(with: jsonData,
            ...
            let apiData = KrakowApiResponse(json: jsonDictionary) else {
        ...
        // map the data
        let trueVehicles = apiData.vehicles.map(mapping)
                                           .flatMap{ $0 }
        
        let dateAsString = KrakowAPI.DateFormatter.string(from: apiData.lastUpdate)
        let convertedToTData = trueVehicles.map{ TData(vehicle: $0, date: dateAsString) }
