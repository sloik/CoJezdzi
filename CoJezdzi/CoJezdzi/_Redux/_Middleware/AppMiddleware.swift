    ...
    static let Api: Middleware<AppState> = { dispatch, getState in
        return { next in
            ...
            return { action in
                ...
                let factory = { return FetchBussesAction(fetched: $0) }
                
                if fetchBusses {
                    WarsawApi.getBusses { handleResult(result: $0, actionFactory: factory, dispatch: dispatch) }
                }
                
                if fetchTrams {
                    WarsawApi.getTrams { handleResult(result: $0, actionFactory: factory, dispatch: dispatch) }
                }
            }
        }
    }
